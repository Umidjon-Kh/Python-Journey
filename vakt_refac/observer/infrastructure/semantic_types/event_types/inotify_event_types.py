...
class InotifyEventType(EventType):
    ...
    FILE_CLOSED_WRITE which fires once after a file is fully written
    and closed — eliminating the noise of intermediate write events
    that FILE_MODIFIED would produce on every partial write.

    Notes:
        - Produced only by InotifyWatcher.
