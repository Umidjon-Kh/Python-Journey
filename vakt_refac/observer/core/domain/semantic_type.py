...
class SemanticTypesMeta(type):
    ...
    This enables subclassing of SemanticType without the restrictions
...
class SemanticType(str, metaclass=SemanticTypesMeta):
    ...
    Why metaclass approach:
        ...
        into SemanticType instances at class creation time. This means:
            - isinstance(EventType.FILE_CREATED, SemanticType)    → True
            - isinstance(InotifyEventType.FILE_ACCESSED, SemanticType) → True
            - isinstance(InstructionType.BACKUP, SemanticType)    → True
    ...
    Notes:
        ...
        - Subclasses automatically inherit this behavior through
            SemanticTypesMeta without any additional configuration.
        - Do not add members to SemanticType directly - create
            domain-specific subclasses for each semantic category.
    """
