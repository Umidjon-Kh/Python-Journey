...
class BaseWatcher(PortProtocol):
    ...
    Common protocol between Assembler and implementations:
        ...
        the correct and coordinated operation:
            ...
            - ignoring_paths: a shared reference-counting mapping that signals which paths
                ...
                ignoring_paths with a count greater than zero. If so, the event must be
    ...
    Ignoring Paths Protocol:
        The Watcher is the only place where ignoring_paths filtering occurs. Filtering
        ...
        Before enqueuing any event into the buffer, the Watcher must check:
            if ignoring_paths.get(event.path, 0) > 0:
                skip  # self-generated event, drop silently

        The Watcher must never modify ignoring_paths itself — only read it.
    ...
    Notes:
        ...
        - It must never modify ignoring_paths — only read it for filtering.
    """
    ...
    def events(self) -> None:
        ...
        Checks ignoring_paths before enqueuing each event — any event whose
