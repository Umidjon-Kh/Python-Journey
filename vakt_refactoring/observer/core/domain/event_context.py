...
class EventContext:
    ...
    Handler Loop Mechanics:
        ...
        - handlers_count: total number of handlers that will process this event.
            ...
            themselves from the termination condition. Used together with
            processed_handlers to determine loop termination.
        - performed: a sequence of InstructionType values that represent publicly
            completed actions. Not every handler appends to performed - only
            those whose completion needs to be observed by other handlers as
            a dependency signal. For example, BackupInvoker appends
            InstructionType.BACKUP so that SnapshotRotator can check whether
            a backup was actually created before acting.
        - metadata: an open key-value buffer for inter-handler communication.
            Handlers may read and write arbitrary data keyed by any hashable
            value. This is the primary channel for passing rich results between
            handlers. For example, BackupInvoker stores the created Snapshot
            object in metadata so that other handlers can access it without
            coupling to a specific field on EventContext.
